...
int main() {
	...
	write_string("loading\0", (1280/2) - ((8/2) * LETTER_FULL_WIDTH), (720/2) - LETTER_HALF_WIDTH, 0x0fff);
	...
	while (1) {
		...
		write_string("push\nbutton\nto\ncompute\ndepth\0", (1280/2) + LETTER_HALF_WIDTH, (720/6), 0x0fff);
...
//				u16 scaled = (u16) min_int(dist / 20, 0xf);
				u16 scaled = (u16) min_int(dist / 15, 0xf);
		...
		// shift to view phase
		next_state();
		write_string("push button\0", (1280/2) - ((14/2) * LETTER_FULL_WIDTH), 480 + LETTER_HALF_WIDTH, 0x0fff);
		write_string("to reset\0", (1280/2) - ((10/2) * LETTER_FULL_WIDTH), 480 + LETTER_FULL_WIDTH + (2*LETTER_HALF_WIDTH), 0x0fff);
